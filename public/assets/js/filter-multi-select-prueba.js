(function($) {
    ...
    var FilterMultiSelect = function() {
        function FilterMultiSelect(selectTarget, args) {
            ...
            this.documentKeydownListener = function(e) {
                ...
                case "Tab":
                    ...
                    if (_this.hasOwnProperty("extraTools")) {
                        if (!_this.extraToolsWindow.contains(document.activeElement)) {
            ...
            this.maxNumSelectedItems = !select.multiple ? 1 : args.selectionLimit > 0 ? args.selectionLimit : parseInt(select.getAttribute("multiple")) > 0 ? parseInt(select.getAttribute("multiple")) : 0;
            ...
            this.selectAllOption = restrictSelection ? new FilterMultiSelect.RestrictedSelectAllOption(this,name,args.selectAllText) : new FilterMultiSelect.UnrestrictedSelectAllOption(this,name,args.selectAllText);
            ...
            //Parte donde se agregan las herramientas extra y su contenedor
            if (args.hasOwnProperty("addElementFormHTML")) {
                this.extraTools = document.createElement("div");
                this.btnAddItem = document.createElement("button");
                this.btnAddItem.type = "button";
                this.btnAddItem.textContent = "Agregar elemento";
                this.extraTools.append(this.btnAddItem);
    
                this.extraToolsWindow = document.createElement("div");
                
                this.extraToolsWindowHeader = document.createElement("div");
                this.extraToolsWindowHeaderTitle = document.createElement("p");
                this.extraToolsWindowHeaderTitle.textContent = "Agregar elemento";
                this.extraToolsWindowHeaderBack = document.createElement("button");
                this.extraToolsWindowHeaderBack.type = "button";
                this.extraToolsWindowHeaderBackIcon = document.createElement("i");
                this.extraToolsWindowHeaderBack.append(this.extraToolsWindowHeaderBackIcon);
                this.extraToolsWindowHeader.append(this.extraToolsWindowHeaderBack, this.extraToolsWindowHeaderTitle);
                
                this.extraToolsWindowBody = document.createElement("div");
                this.extraToolsWindowBody.innerHTML = args.addElementFormHTML;
    
                this.extraToolsWindow.append(this.extraToolsWindowHeader, this.extraToolsWindowBody);

                this.addElementFormSubmitHandler = args.addElementFormSubmitHandler;
            }
            ...
            if (args.hasOwnProperty("addElementFormHTML")) {
                this.dropDown.append(this.extraTools, this.extraToolsWindow);
            }
            ...
            this.createOption = FilterMultiSelect.createOption;
            this.filterText = "";
        ...
        ;
        FilterMultiSelect.createOption = function(fms, name, htmlOption, type = "html") {
            var newoption = null;

            if (type === "html") {
                // HTML Option
                FilterMultiSelect.checkValue(htmlOption.value, htmlOption.label);
                newoption = new FilterMultiSelect.SingleOption(fms,fms.options.length,name,htmlOption.label,htmlOption.value,htmlOption.defaultSelected,htmlOption.disabled)
            } else if (type === "js") {
                // JS Option
                var label = jsOption[0];
                var value = jsOption[1];
                var selected = jsOption[2];
                var disabled = jsOption[3];
                
                FilterMultiSelect.checkValue(value, label);

                newoption = new FilterMultiSelect.SingleOption(fms,fms.options.length + j,name,label,value,selected,disabled);
            }
            
            // TODO: Validar si ya existe el valor
            newoption.initialize();
            fms.items.append(newoption.getListItem());
            fms.options.push(newoption);

            if (newoption.checkbox.checked) {
                newoption.select();
            }

            fms.maxNumSelectedItems = fms.maxNumSelectedItems + 1;
            fms.showing[fms.options.length] = fms.options.length;

            if (fms.hasOwnProperty("extraTools")) {
                fms.closeExtraToolsWindow();
                fms.extraToolsWindowBody.querySelector("form").reset();
            }            

            fms.dispatchCreatedOptionEvent(newoption);
        ...
        ;
        FilterMultiSelect.createCustomEvent = function(e, option) {
            var event = new CustomEvent(e,{
                detail: {
                    option
                },
        ...
        FilterMultiSelect.prototype.initialize = function() {
            ...
            if (this.hasOwnProperty("extraTools")) {
                this.extraTools.className = "extra-tools dropdown-item";
                this.btnAddItem.className = "btn btn-sm btn-primary w-100";
                this.extraToolsWindow.className = "extra-tools-window";
                this.extraToolsWindowHeader.className = "extra-tools-window-header";
                this.extraToolsWindowHeaderBackIcon.className = "fas fa-chevron-left";
                this.extraToolsWindowBody.className = "extra-tools-window-body";

                this.attachBtnAddItemListener();
                this.attachExtraToolsBtnBackListener();
                this.attachExtraToolsFormSubmitHandler();
            }
        ...
        ;
        FilterMultiSelect.prototype.openExtraToolsWindow = function() {
            this.filterInput.disabled = true;

            this.extraToolsWindow.classList.add("open");
            this.extraToolsWindowHeaderBack.focus();
        }
        ;
        FilterMultiSelect.prototype.closeExtraToolsWindow = function() {
            this.extraToolsWindow.classList.remove("open");
            this.filterInput.disabled = false;
        }
        ;
        FilterMultiSelect.prototype.attachBtnAddItemListener = function() {
            this.btnAddItem.addEventListener("click", (e) => {
                e.stopImmediatePropagation();

                this.openExtraToolsWindow();
            });
        }
        ;
        FilterMultiSelect.prototype.attachExtraToolsBtnBackListener = function() {
            this.extraToolsWindowHeaderBack.addEventListener("click", (e) => {
                e.stopImmediatePropagation();

                this.closeExtraToolsWindow();
            });
        }
        ;
        FilterMultiSelect.prototype.attachExtraToolsFormSubmitHandler = function() {
            var _this = this;
            this.extraToolsWindowBody.querySelector("form").addEventListener("submit", (e) => {
                this.addElementFormSubmitHandler(e, _this);
            });
        }
        ;
        ...
        FilterMultiSelect.prototype.updateDropdownList = function() {
            ...
            let regExpBusqueda = new RegExp(`(${text.normalize('NFD').replace(regexAcentosNFD,"")})`, this.caseSensitive ? 'g' : 'gi');

            this.options.forEach(function(o, i) {
                if (o.getLabel().normalize('NFD').replace(regexAcentosNFD,"").match(regExpBusqueda)) {
                    o.show();
                    showing.push(i)
                } else {
                    o.hide()
        ...
        FilterMultiSelect.prototype.attachViewbarListeners = function() {
            ...
            this.viewBar.addEventListener("key", function(e) {
                if (_this.isClosed()) {
                    _this.openDropdown()
                } else {
                    _this.closeDropdown()
        ...
        FilterMultiSelect.prototype.incrementItemFocus = function() {            
            if ((this.hasOwnProperty("extraTools"))) {
                if (this.itemFocus >= this.options.length) {
                    return;
                }
            } else {
            ...
            var i = this.itemFocus; //  El numero actual del elemento enfocado
            do {
                i++
            } while (
                // Sumamos 1 a la variable i hasta que
                // i == -1 y el botón de seleccionar todos  este deshabilitado o escondido
                // ó i >= 0 y i sea menor que el numero de opciones y el numero de opción este deshabilitada u oculta
                (i == -1 && (this.selectAllOption.isDisabled() || this.selectAllOption.isHidden())) ||
                (i >= 0 && i < this.options.length && (this.options[i].isDisabled() || this.options[i].isHidden()))
            );

            if (this.hasOwnProperty("extraTools")) {
                this.itemFocus = (i > this.options.length) ? this.itemFocus : i;
            } else {
        ...
        FilterMultiSelect.prototype.decrementItemFocus = function() {
            ...
            do {
                i--
            } while (i == -1 && (this.selectAllOption.isDisabled() || this.selectAllOption.isHidden()) || i >= 0 && (this.options[i].isDisabled() || this.options[i].isHidden()) && i > -2);
            this.itemFocus = i;
        }
        ;
        FilterMultiSelect.prototype.focusItem = function() {
            if (this.itemFocus === -2) {
                ...
                } else if ((this.itemFocus === this.options.length) && (this.hasOwnProperty("extraTools"))) {
        ...
        ;
        FilterMultiSelect.prototype.dispatchCreatedOptionEvent = function(option) {
            var event = FilterMultiSelect.createCustomEvent(FilterMultiSelect.EventType.OPTION_CREATED, option);
