...
@section('title', 'Editar categoría de excursión')

@section('content_header')
    <h1><span class="font-weight-bold">CloudTravel</span> | Editar categoría de excursión</h1>
@stop
...
@section('content')
    <div class="row mb-2">
        <div class="col-md-6 col-sm-6">
            <div class="form-group">
                ...
                <input type="datetime-local" class="form-control form-control-sm" value="{{ $categoria->created_at }}" disabled>
            </div>
        </div>
        ...
        <div class="col-md-6 col-sm-6">
            <div class="form-group">
                ...
                <input type="datetime-local" class="form-control form-control-sm" value="{{ $categoria->updated_at }}" disabled>
            </div>
        </div>
    </div>
    ...
    <div class="card card-info">
        ...
        <div class="card-body">
            <form id="editar-categoria-excursion" action="{{ route('categorias.update', $categoria->id) }}" method="post">
                @csrf
                @method('put')
                ...
                <div class="row">
                    <div class="col-md-6 col-sm-6">
                        <div class="form-group">
                            <label>Nombre</label>
                            <input type="text" required name="nombre" class="form-control" value="{{ $categoria->nombre }}" placeholder="Ingresar nombre">
                        </div>
                    </div>
                    ...
                    <div class="col-md-6 col-sm-6">
                        <div class="form-group">
                            ...
                            <textarea name="descripcion" class="form-control" placeholder="Agregar una descripción de la categoría de lan excursión" style="height: 100px" required>{{ $categoria->descripcion }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-12 col-sm-12">
                        <div id="editar-categoria-dropzone" class="dropzone @php echo ($categoria->imagenes->count() > 0) ? "dz-started" : "" @endphp">
        ...
        <div class="card-footer d-grid gap-2 d-md-flex justify-content-md-end p-3">
            <button form="editar-categoria-excursion" class="btn btn-primary mx-3" type="submit">
...
@section('js')
    <script>
        ...
        function eliminarImagen(event, idModeloParaEliminar) {
            ...
            fetch(`./admin/categorias/{{ $categoria->id }}/imagenes/${idModeloParaEliminar}`, {
                    method: "POST",
                    body: formulario
                })
                .then(res => res.json())
        ...
        function marcarImagen(event, idModeloImagenFavorita) {
            ...
            fetch(`./admin/categorias/{{ $categoria->id }}/imagenes/${idModeloImagenFavorita}`, {
                    method: "POST",
                    body: formulario
                })
                .then(res => res.json())
    ...
    <script>
        $(document).ready(function() {
            // Dropzone
            let newDropZone = new Dropzone(`#editar-categoria-dropzone`, {
                url: "./admin/categorias/{{ $categoria->id }}/imagenes",
                headers: {
            ...
            $('#modalEditarImagen').on('show.bs.modal', function (event) {
                ...
                .then(function(response) {
                    ...
                    modalBody.innerHTML = `
                    <x-generales.imagenes.formularios.editar action="{{ route('categorias.index') }}/{{ $categoria->id }}/imagenes/${imgId}" path="${imgElement.src}" className="form-modal" btnSubmit="false" imagen-id="${imgId}" titulo="${imgTitulo}" descripcion="${imgDescripcion}" leyenda="${imgLeyenda}" texto-alternativo="${imgTextoAlternativo}">
                        <x-slot:imagen-info>
