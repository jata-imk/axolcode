...
namespace App\Policies\Promociones;
...
class PromocionPolicy
{
    ...
    public function verificarPermisosModulo(Usuario $usuario, $accion = "ver")
    {
        if (($usuario->id_rol === 0) || ($usuario->rol->nombre === "Administrador")) {
            ...
            $menus = Menu::leftJoin("roles_menus", "menus.id", "=", "roles_menus.id_menu")
                ...
                ->where("menus.url", "=", "admin/promociones")
                ->select("menus.*")
                ->get();
     ...
     *
     * @param  \App\Models\Usuario  $usuario
     * @param  \App\Models\Promociones\Promocion $promocion
     * @return \Illuminate\Auth\Access\Response|bool
     */
    public function view(Usuario $usuario, Promocion $promocion)
    {
        ...
        return $usuario->id_empresa === $promocion->id_empresa
            ? Response::allow()
            : Response::deny('Esta promoción no pertenece a esta empresa');
    }
     ...
     * @return \Illuminate\Auth\Access\Response|bool
     */
    public function update(Usuario $usuario, Promocion $promocion)
    {
     ...
     * @return \Illuminate\Auth\Access\Response|bool
     */
    public function delete(Usuario $usuario, Promocion $promocion)
    {
