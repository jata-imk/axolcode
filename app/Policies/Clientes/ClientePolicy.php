...
namespace App\Policies\Clientes;
...
class ClientePolicy
{
    ...
    public function verificarPermisosModulo(Usuario $usuario, $accion = "ver")
    {
        if (($usuario->id_rol === 0) || ($usuario->rol->nombre === "Administrador")) {
            ...
            $menus = Menu::leftJoin("roles_menus", "menus.id", "=", "roles_menus.id_menu")
                ...
                ->where("menus.url", "=", "admin/clientes")
                ->select("menus.*")
                ->get();
    ...
    public function view(Usuario $usuario, Cliente $cliente)
    {
        ...
        return $usuario->id_empresa === $cliente->id_empresa
            ? Response::allow()
            : Response::deny('Este cliente no pertenece a esta empresa');
    }
